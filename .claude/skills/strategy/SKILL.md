...
- **核心数据点** (数智生活商城参考示例 — 替换为您项目的数据):
  - 核心框架："一基+双核+两翼"打造运营飞轮，实现价值增长
  - 一基：搭建好一个基座 — 数智生活商城，聚焦客户八大用户需求
  - 双核：利用好两个核心载体 — 数智豆(商城统一货币体系) + 支付码(身份码/权益码/支付码三码合一)
  - 两翼：发展好两类会员模式 — 套餐回馈型会员 + 增值付费型会员(Plus付费会员)
  - 目标：实现从用户规模到平台流量的价值经营
...
**通过 `/learn-strategy` 触发**

执行步骤:
1. 扫描 `news/` 目录，读取所有 `.md` 文件
2. 按关键词评分相关性，关键词: 策略,一基双核两翼,飞轮,数智豆,支付码,会员,价值增长
...
（由 `/learn-strategy` 从 `news/` 目录填充，初始为空）
