...
- **章节标题**: 推进三种模式转变
- **核心数据点**:
  - 合作模式转变：BC(单一销售)→BBC(生态协同，平台运营为核心联合多方企业伙伴)→NBC(数智生态，以数智化商业网络N为底座)
  - 产品模式创新：领取模式→多形态产品(数智会员/商城货架/消费折扣/精品团购/权益豆/权益券)
  - 商业模式升级：按量结算→多元模式结合(资源对投/平台分润/后向收入)
  - 案例1：江苏公司BBC运营 — 联合洋河酒厂+江苏银行实现资源互通渠道共享
  - 案例2：爆米花会员 — 5元×4张免费膨胀、叠加兑换，20元超会员¥9.9开通
  - 案例3：中旅CDF网龄福利主题日 — UV68.1万、参与用户9.4万、CDF权益券总领取27.1万、带动产品销售超800万元
...
**通过 `/learn-business-model` 触发**

执行步骤:
1. 扫描 `news/` 目录，读取所有 `.md` 文件
2. 按关键词评分相关性，关键词: 商业模式,BC,BBC,NBC,合作模式,产品模式,模式转变,生态
...
（由 `/learn-business-model` 从 `news/` 目录填充，初始为空）
