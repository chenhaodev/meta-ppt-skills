...
name: resources
description: >
  TRIGGER when user invokes /resources or asks to analyze or write the 资源保障 section
...
- **核心数据点** (数智生活商城参考示例 — 替换为您项目的数据):
  - 数智豆资源：保障数智豆发放规模，划拨营销成本预算，专款专用，与数智生活会员/全球通等深度融合
  - 积分及金币资源向数智豆迁移，实现客户数字资产归口管理
  - 预付款资源：统筹约10亿元预付款资源，用于买断包销等，获取商户最低折扣
  - 客户运营资源：统筹运营资源，将业务利润的约3%用于数智生活商城拉新促活
...
**通过 `/learn-resources` 触发**

执行步骤:
1. 扫描 `news/` 目录，读取所有 `.md` 文件
2. 按关键词评分相关性，关键词: 资源,数智豆,预付款,客户运营,资源配置,资源保障
...
（由 `/learn-resources` 从 `news/` 目录填充，初始为空）
