...
# tech-architecture: 商城平台技术架构 专家技能
...
- **核心数据点** (数智生活商城参考示例 — 替换为您项目的数据):
  - 架构目标：以数字化、智能化重构商业生态，构建敏捷化、多元化开放能力
  - IaaS基础设施层：集中化基础设施(通算/智算/存储/网络) + 省专分布式基础设施
  - T-PaaS基础平台层：AI原生(AI Agent Runtime Platform) + 元数据多租(Metadata Multi-Tenancy) + 通用技术平台(Common Technology Platform)
  - A-PaaS产品能力层：权益产品中心/权益服务中心/运营管理中心/行业共享运营中心/能力中台运营中心
  - B-PaaS敏捷编排层：产品引擎/流程引擎/接入引擎/UI引擎/分析引擎/低代码编排引擎
  - SaaS业务应用层：车主服务/康养家庭/年轻客群/旅游出行/宠物场景/美食商超/海外专区
  - MCP开放平台(在通用技术平台中)
...
**通过 `/learn-tech-architecture` 触发**

执行步骤:
1. 扫描 `news/` 目录，读取所有 `.md` 文件
2. 按关键词评分相关性，关键词: 技术架构,IaaS,PaaS,SaaS,AI原生,元数据,敏捷编排,MCP,低代码
...
（由 `/learn-tech-architecture` 从 `news/` 目录填充，初始为空）
