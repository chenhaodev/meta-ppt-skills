...
- **核心数据点** (数智生活商城参考示例 — 替换为您项目的数据):
  - 提升产品力：一豆(数智豆货币体系) + 一码(三码合一) + 一基座(平台能力底座)
  - 提升运营力：面向客户四个升级(数智资产集中化/交互体验极致化/消费高频化/口碑传播客户化)
  - 提升运营力：面向生态四个赋能(客户赋能/数据赋能/资源赋能/工具赋能)
  - 提升销售力：六大运营阵地(中国移动APP/社区/泛全/政企/农村/校园)
  - 提升支撑力：统一体系框架+两级运营支撑+数据分析支撑
  - 产品营销计划："一二三四"营销模式(一体化策划/两维清单/三类场景/四轮驱动)
...
**通过 `/learn-operations` 触发**

执行步骤:
1. 扫描 `news/` 目录，读取所有 `.md` 文件
2. 按关键词评分相关性，关键词: 运营,产品力,运营力,销售力,支撑力,四个升级,四个赋能,六大阵地
...
（由 `/learn-operations` 从 `news/` 目录填充，初始为空）
