...
- **核心数据点** (数智生活商城参考示例 — 替换为您项目的数据):
  - 权益超市：权益订购客户从0.78亿→3.10亿(+297%)，权益收入从34.13亿→274.2亿(+705%)
  - 积分商城：积分客户9.1亿，年交易规模306亿(+356%)，客户满意度90分
  - 公司十大优势：客户规模、基础设施、数据要素、品牌资产、多元生态、财金实力、数智内容、人工智能、科技创新、人才队伍
  - 数智生活商城差异化竞争：客户为什么来(价格便宜/玩法丰富/体验便捷/一站式服务)
  - 合作伙伴为什么来(有增量空间+有资源加持)
...
**通过 `/learn-self-analysis` 触发**

执行步骤:
1. 扫描 `news/` 目录，读取所有 `.md` 文件
2. 按关键词评分相关性，关键词: 权益,积分,自身,优势,成效,商城,差异化
...
（由 `/learn-self-analysis` 从 `news/` 目录填充，初始为空）
